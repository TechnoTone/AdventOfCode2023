...
describe("chunkRange", () => {
  ...
  test("splits the range in 2 where there as a LEFT overlap", () => {
    ...
    expect(chunkRange(range, mappingTable)).toStrictEqual([{ start: 2, end: 4 }, { start: 5, end: 7 }])
  })
  test("splits the range in 2 where there as a RIGHT overlap", () => {
    const range = { start: 27, end: 37 }
    expect(chunkRange(range, mappingTable)).toStrictEqual([{ start: 27, end: 30 }, { start: 31, end: 37 }])
  })
  ...
  test("splits the range in to 3 when there are BOTH LEFT AND RIGHT overlaps on a single mapTransformation", () => {
    ...
    expect(chunkRange(range, mappingTable)).toStrictEqual([{ start: 22, end: 24 }, { start: 25, end: 30 }, { start: 31, end: 33 }])
  })
